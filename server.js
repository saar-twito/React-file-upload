...
app.post('/upload', (req, res) => {
  ...
  if (!allowedFormats.includes(format[1]))
    return res.status(400).send('Image has to be (png, jpg, jpeg, tiff)');


  // Check the size of the image not ove then 1.5 MB
  if (file.size >= 1500000)
    return res.status(400).send('Image\'s size must be equal or smaller than 1.5 MB');
